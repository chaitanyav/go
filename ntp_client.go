...
func (packet *NTPPacket) decodeStratum() string {
  stratum := ""
  if packet.Stratum == 0 {
    stratum = "unspecified"
...
func (packet *NTPPacket) decodeLeapIndicator() string {
	b := (packet.Byte1 >> 6) & 3
	return leapIndicator[b]
}

func (packet *NTPPacket) decodeVersion() byte {
	return (packet.Byte1 >> 3) & 7
}

func (packet *NTPPacket) decodeMode() string {
	b := packet.Byte1 & 7
  return mode[b]
}

func (packet *NTPPacket) decodeReferenceIdentifier() string {
    return ""
}

func query(packet NTPPacket) (*NTPPacket, error) {
...
func main() {
	packet := NTPPacket{}
	packet.Byte1 = 27
  packet.OriginateTimeStamp = uint64(time.Now().Unix())
	res, err := query(packet)
	if err == nil {
		fmt.Printf("%v\n", res)
	}
	fmt.Println("NTP Version: ", res.decodeVersion())
	fmt.Println("Leap Indicator: ", res.decodeLeapIndicator())
  fmt.Println("Mode: ", res.decodeMode())
  fmt.Println("Stratum: ", res.decodeStratum())
  fmt.Println("Poll: ", res.Poll)
}
